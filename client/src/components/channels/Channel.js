...
class Channel extends Component {
  ...
  componentDidMount() {
    ...
    function fetch_messages(self) {
      axios.get(`https://black-board-engage.herokuapp.com/api/messages?channel=${value}&limit=${MESSAGE_LIMIT}`).then(res => {
        ...
        if (err.response.status === 404) {
          axios.post("https://black-board-engage.herokuapp.com/api/channels", {
            name: value
          }).then(res => {
            fetch_messages(self);
          });
  ...
  doHeartbeat() {
    ...
    axios.get(`https://black-board-engage.herokuapp.com/api/messages?channel=${this.state.channel}&after=${this.state.heartbeat_timestamp.toISOString()}`).then(res => {
  ...
  handleMessageSubmit(event) {
    ...
    axios.post("https://black-board-engage.herokuapp.com/api/messages", {
